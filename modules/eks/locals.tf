locals {
  ...
  charts_with_pod_id = {
    ...
    if contains(["aws-lbc", "external-dns", "karpenter", "generic_pod_identity", "ebs-csi"], c.iam_type)
  }
  ...
  all_role_arns = merge(
    ...
    { for k, v in local.charts_karpenter : k => aws_iam_role.karpenter_controller[k].arn },
